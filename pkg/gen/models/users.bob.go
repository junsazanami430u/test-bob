...
var Users = mysql.NewTablex[*User, UserSlice, *UserSetter]("users", []string{"email"}, []string{"id"}, []string{"id"})
...
var UserErrors = &userErrors{
	ErrUniqueEmail: &UniqueConstraintError{s: "email"},

	ErrUniqueId: &UniqueConstraintError{s: "id"},
...
func (s *UserSetter) Apply(q *dialect.InsertQuery) {
	...
	q.AppendValues(
		bob.ExpressionFunc(func(ctx context.Context, w io.Writer, d bob.Dialect, start int) ([]any, error) {
			...
			return mysql.Arg(s.Email).WriteSQL(ctx, w, d, start)
		}), bob.ExpressionFunc(func(ctx context.Context, w io.Writer, d bob.Dialect, start int) ([]any, error) {
			if s.Password.IsUnset() {
				return mysql.Raw("DEFAULT").WriteSQL(ctx, w, d, start)
			}
			return mysql.Arg(s.Password).WriteSQL(ctx, w, d, start)
		}), bob.ExpressionFunc(func(ctx context.Context, w io.Writer, d bob.Dialect, start int) ([]any, error) {
			if s.CreatedAt.IsUnset() {
				return mysql.Raw("DEFAULT").WriteSQL(ctx, w, d, start)
			}
			...
			return mysql.Arg(s.UpdatedAt).WriteSQL(ctx, w, d, start)
		}))
}
...
func FindUser(ctx context.Context, exec bob.Executor, IDPK []byte, cols ...string) (*User, error) {
	if len(cols) == 0 {
		return Users.Query(
...
func UserExists(ctx context.Context, exec bob.Executor, IDPK []byte) (bool, error) {
	return Users.Query(
		SelectWhere.Users.ID.EQ(IDPK),
	).Exists(ctx, exec)
}
